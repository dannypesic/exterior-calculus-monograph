...
\\\\ Green's Theorem:
\[
\oint_{\p D}{Pdx+Qdy} = \iint_{D}{\left(\vec{\nabla} \times \bv{F}\right) \cdot \uv{k} \cdot dxdy}
\]
Stokes' Theorem:
\[
\oint _{\p S}{\bv{F}\cdot d\bv{r}} = \iint_{S}{\left(\vec{\nabla} \times \bv{F}\right)\cdot \uv{n}\cdot d\bv{S}}
\]
Gauss's Theorem:

\[
\varoiint_{\p V} {\left(\bv{F} \cdot \uv{n} \right)}dS = \iiint_{V}{\left(\vec{\nabla} \cdot \bv{F}\right)dV}
\]
